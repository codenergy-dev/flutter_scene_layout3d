...
Its settings tab carries five colour swatches and a brightness switch, and
they re-theme both Material surfaces while it runs: the gallery draws a scheme
generated by `ColorScheme3d.fromSeed` rather than either hand-written
baseline, which is what an application with a brand does.

The suites are two, and both matter:

```sh
flutter pub get                                        # resolves the workspace
cd packages/flutter_scene_layout3d && flutter test      # 1288, arithmetic
cd packages/flutter_scene_material3d && flutter test    # 585, arithmetic
